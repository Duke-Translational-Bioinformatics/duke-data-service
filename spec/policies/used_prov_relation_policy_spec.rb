...
describe UsedProvRelationPolicy do
  ...
  let(:used_prov_relation) { FactoryGirl.create(:used_prov_relation, relatable_from: activity, relatable_to: file_version) }
  ...
  context 'user created activity and has visibility to the file_version' do
    ...
    it_behaves_like 'a user without project_permission', :view_project, denies: [:show?, :create?, :destroy?], on: :used_prov_relation
  end
