...
describe ProjectPermissionPolicy do
  ...
  context 'when another :manage_project_permissions exists' do
    let!(:another_project_admin) { FactoryGirl.create(:project_permission, :project_admin, project: project_permission.project) }
  ...
  context 'when a non manager exists' do
    let(:non_admin_role) { FactoryGirl.create(:auth_role, without_permissions: [:manage_project_permissions]) }
    let!(:a_non_admin) { FactoryGirl.create(:project_permission, auth_role: non_admin_role, project: project_permission.project) }
  ...
  context 'when a :manage_project_permissions exists in different project' do
    let!(:another_project_admin) { FactoryGirl.create(:project_permission, :project_admin) }
