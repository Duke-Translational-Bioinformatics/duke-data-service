...
RSpec.describe ChildRestorationJob, type: :job do

  shared_examples 'a ChildRestorationJob' do |
      parent_sym,
      child_folder_sym,
      child_file_sym
    |
    ...
    it { expect(described_class.queue_name).to eq("#{prefix}#{prefix_delimiter}child_restoration") }
    it {
      expect {
    ...
    context 'perform_now' do
      ...
      it {
        ...
        expect(parent).to receive(:restore_children).with(page)
  ...
  it_behaves_like 'a ChildRestorationJob', :folder, :sub_folder, :sub_file
end
