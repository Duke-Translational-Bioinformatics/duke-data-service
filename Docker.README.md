...
* BASE_URI: The Base URI to the live Duke Authentication Service, e.g. URI/api/v1/app/status should return {'status': 'ok'}
* NAME: A Description of the Service used in the apiexplorer and portal frontends
...
In addition, the following environment variables must also be set on servers that
...
Finally, Consumers must be registered with the live Duke Authentication Service (See Documentation in the [Duke Authentication Service](https://github.com/Duke-Translational-Bioinformatics/duke-authentication-service)
for more information about registering consumers).
...
* BASE_URI: see [Configuration](#connecting-a-duke-authentication-service-microservice)
* NAME: see [Configuration](#connecting-a-duke-authentication-service-microservice)
