...
* `results`: these do not need to be analyzed in any specific way, unless they are query_string.field, filter, or post_filter. We should include only the information that we want to be returned, or filtered. We can add new fields to the index as we expand search functionality, or desired result data. This will optimize performance of indexing (and rebuilding all indexes for) documents.
